...
//making a funciton to check if sample is same as the candidate, return 1 or 0

int check_acidtype_same (char *sample_segment, char * candidate_segment){
  int m = get_codon_index(sample_segment);
  int n= get_codon_index(candidate_segment);
  
  if(strcmp(codon_names[m], codon_names[n]) == 0){
    return 1;
  }
...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates, char* output_string)
{
    ...
    for (int m = 0; m < number_of_candidates; m++) {
        ...
        candidate_length = strlen(candidate_segments[m]);

        outputline_buffer[0] = '\0';
        int_buffer[0] = '\0';

        if (sample_length == candidate_length) {
            if (strcmp(sample_segment, candidate_segments[m]) == 0) {
                has_perfect_match++;

                sprintf(int_buffer, "%d", m + 1);
                strcat(outputline_buffer, "Candidate number ");
                strcat(outputline_buffer, int_buffer);
                strcat(outputline_buffer, " is a perfect match\n");
        ...
        if (has_perfect_match == 0) {
            ...
            for (int k = 0; k < number_of_candidates; ++k) {
                ...
                score = calculate_score(sample_segment, candidate_segments[k]);
                //output 
                sprintf(int_buffer, "%d", k + 1);
                ...
                int length = strlen(outputline_buffer);

               for (j = 0; j < length; j++) {
                    *(output_string + j) = *(outputline_buffer+j);
                }
...
int calculate_score(char* sample_segment, char* candidate_segment) {
    ...
    int bigloop_to_repeat = candidate_length_in_codons - sample_length_in_codons;

    for (int k = 0; k <= bigloop_to_repeat; k++) {
        ...
        for (int i = 0; i < sample_length_in_codons; i++) {
            ...
            if (strncmp( (sample_segment + (CODON_LENGTH*i)) , (candidate_segment + (i+k)*CODON_LENGTH), CODON_LENGTH ) == 0) {
                temp_score += 10;
            }
            //check both segment exist in the list, each codon
            else if (check_acidtype_same(sample_segment+(i*CODON_LENGTH) , candidate_segment+(i+k)*CODON_LENGTH) == 1){
                temp_score +=5;
            }
                    ...
                    for (int j = 0; j < CODON_LENGTH; j++) {
                        ...
                        if (sample_segment[j + i * CODON_LENGTH] == candidate_segment[j + (i + k) * CODON_LENGTH]) {
                          temp_score +=2;
                        }
                        else if (is_base_pair(sample_segment[j + i * CODON_LENGTH], candidate_segment[j + (i + k) * CODON_LENGTH]) != 0)
                            temp_score += 1;
                    }
                }
