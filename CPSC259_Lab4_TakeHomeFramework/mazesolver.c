...
void process()
{
	...
	sprintf(outputstring, "Total number of solution %d\n", paths_found);
	*(outputstring + 1) = '\0';
...
void generate_all_paths(char*** pathsetref, int* numpathsref, maze_cell** maze, int dimension, int row, int column, char* path)
{
	...
	if (row > dimension - 1 || column < 0 || row < 0 || column > dimension - 1 || maze[row][column].character == MAZE_WALL || maze[row][column].visited == VISITED) {
...
void construct_shortest_path_info(char** pathset, int numpaths, char* outputbuffer)
{
	...
	*outputbuffer= '\0';
	...
	for(int j=0; j< numpaths; j++){
		if(strlen(pathset[j]) < strlen(shortest))
		shortest = pathset[j];
	}
...
void construct_cheapest_path_info(char** pathset, int numpaths, char* outputbuffer)
{
	char* cheapest;
	cheapest = pathset[0];
	int cheapest_c = path_cost(cheapest);
	*outputbuffer ='\0';  //initilize the output
 
	char tmp[BUFFER];

	for(int k=0; k< numpaths; k++){
		if(path_cost(pathset[k]) < cheapest_c){
			cheapest_c = path_cost(pathset[k]);
			cheapest = pathset[k];
		}
	}

	sprintf(outputbuffer,"Cheapest path: %s\n", cheapest);
	sprintf(tmp,"Cheapest path cost: %d\n", cheapest_c);
