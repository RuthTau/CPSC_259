...
Author:       Ruth Tau and Tessa Clemen
...
void run_analysis()
{
  ...
  for (int j = 0; j < NUMBER_OF_READINGS; j++) {
      omega[j] = (double)j * (double)SAMPLING_FREQUENCY /(double) NUMBER_OF_READINGS;
  }
  ...
  if (file_pointer != NULL)
      fclose(file_pointer);
                                            ...
                                            // fft(...
  short int j = 1;
  fft(j, EXPONENT, readings, complex_component);
                                            ...
                                            */
                                            ...
                                            // 	readings[i] = ...
  for (int j = 0; j < NUMBER_OF_READINGS; j++) {
      readings[j] =sqrt( (pow(readings[j], 2) + pow(complex_component[j], 2)));
  }
                                            /*
  ...
  for (i = 0; i < (NUMBER_OF_READINGS / 2) ; i++)
  {
      ...
      if (readings[i] > amplitude && omega[i] >= NOISE_FILTER ) {
          frequency = omega[i];
          amplitude = max;
      }
